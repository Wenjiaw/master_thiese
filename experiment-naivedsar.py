...
import numpy as np
from algorithms.dsar_naive import NAIVE_DSAR
...
plt.figure(1)
plt.title('naive-DSAR m = 10', fontweight='bold')
plt.xlabel('t')
plt.ylabel('min')
plt.plot(result[0],'b-',color='r',label='naive DSAR')
plt.legend()
plt.xlim(0,steps)
plt.ylim(0.5,0.9)
plt.savefig('lin_naiveDSAR_min.png')
plt.figure(2)
plt.title('naive-DSAR m = 10', fontweight='bold')
plt.xlabel('t')
plt.ylabel('sum')
plt.plot(result[1],'b-',color='r',label='naive DSAR')
plt.legend()
plt.xlim(0,steps)
plt.ylim(7.8,9.0)
plt.savefig('lin_naiveDSAR_sum.png')
